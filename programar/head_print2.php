...
$id_head = $_GET['id'];
     $query1="
SELECT head_programaciones.id_head, head_programaciones.head_nombre, clientes.cte_nombrecomercial, clientes.cte_logo, head_programaciones.hora_cita, clientes.id_cliente, head_programaciones.fechareg, head_programaciones.head_fecha
FROM head_programaciones INNER JOIN clientes ON head_programaciones.id_cliente = clientes.id_cliente
WHERE (((head_programaciones.id_head)=$id_head) AND ((head_programaciones.fechareg)='$hoy'));
				";
  ...
  <h5 class="card-title"><B> <span class="icon-road"></span> VIAJES PROGRAMADOS:</B> </h5>
   ...
   <p class="card-text"> 
	...
	<?php while($filas1=mysqli_fetch_assoc($result1)) { ?>
               ...
               ▪  <mark> OTR: 000<?php echo $filas1 ['id_head'];?>  
               -  <?php echo $filas1 ['cte_nombrecomercial'];?> / H. CITA: <?php echo $filas1 ['hora_cita'];?></mark>  &nbsp  <br>

                                      <?php
								     $query2="
											SELECT programaciones.id_head, programaciones.id_prog, cuentas.cta_nombrecomercial, unidades.vh_placa AS va, unidades_1.vh_placa AS vr, usuarios.user_nick AS conductor, usuarios_1.user_nick AS ayudante, tipo_prog.tprog_nombre, habilidad.nom_habilidad
